...
if use_managed_deps:
    ...
    if USE_SERVERLESS:
        ...
        print("Connected to cluster <cluster_id> with managed dependencies!")
else:
    ...
    if USE_SERVERLESS:
        ...
        print("Connected to cluster <cluster_id>")
...
customers_df = spark.range(0, N_CUSTOMERS, numPartitions=PARTITIONS).select(
...
customers_df = customers_df.withColumn(
    "arr", F.round(generate_lognormal_amount(F.col("tier")), 2)
...
orders_df = spark.range(0, N_ORDERS, numPartitions=PARTITIONS).select(
...
orders_with_fk = orders_df.join(customer_lookup_with_idx, on="customer_idx", how="left").drop(
    "customer_idx"
)

# Add tier-based amount
orders_with_fk = orders_with_fk.withColumn(
    "amount", F.round(generate_lognormal_amount(F.col("tier")), 2)
)
