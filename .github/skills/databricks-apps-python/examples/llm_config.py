...
def _is_token_fresh(cache: MutableMapping[str, Any] | dict[str, Any]) -> bool:
    return bool(
        cache.get("access_token") and int(cache.get("expires_at", 0)) > int(time.time()) + 30
    )
...
def get_databricks_bearer_token(
    ...
    with _token_lock:
        ...
        if not access_token:
            ...
            raise DatabricksLLMConfigError(
                "Token endpoint response is missing access_token " f"(keys present: {payload_keys})"
            )
...
def build_openai_client(
    *,
    ...
    config = (
        validate_databricks_llm_config(cache=cache) if validate else get_databricks_llm_config()
    )
    token = get_databricks_bearer_token(cache=cache)
...
def run_jobs_parallel(
    ...
    if worker_count < 1:
        raise DatabricksLLMConfigError("LLM_MAX_CONCURRENCY must be a positive integer.")
